...
export const useWrite = (origin: 'event' | 'recruitment') => {
  const router = useRouter();
  return useMutation({
    mutationFn: (props: { apiURL: string, body: Object }) => { return mutateForURLJson(props.apiURL, 'POST', props.body) },
    onSuccess: data => {
      if (data.errorCode === 'C00000') {
        alert('성공적으로 게시되었습니다.')
        router.push(`/${origin}/${data.data.id}`)
      }
