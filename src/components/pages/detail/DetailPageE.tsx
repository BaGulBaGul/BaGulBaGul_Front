...
export function DetailPageE({ postId }: { postId: any; }) {
  ...
  return (
    <>
      <DetailWrapper title={typeString[data.event.type as string]}>
      ...
      {data.event.type !== 'PARTY' && <FooterButton text={"모집글 보러가기"} path={`/event/${data.event.eventId}/recruitment`} />}
      <BottomDrawer open={openD} toggleDrawer={toggleDrawer} opt='EVT' target={data.event.eventId}
        me={data.event.type === 'PARTY' && !!userinfo && userinfo.id === data.post.writer.userId} handleDelete={handleDelete}
        handleEdit={data.event.type === 'PARTY' ? undefined : () => router.push(`/write?w=p&edit=${data.event.eventId}`)} />
    </>
  );
}