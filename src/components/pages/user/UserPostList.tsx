...
import { Divider, ListProps, LoadingCircle, MoreButton, RListProps, SkeletonList } from "@/components/common";
...
export function UserPostList({ value, me, userinfo }: { value: 0 | 1, me: boolean, userinfo: any }) {
  ...
  if (posts.isPending || posts.isLoading) { return <SkeletonList type='POST' opt={value === 0 ? 'EVT' : 'RCT'} /> }
  else if (!posts.data || (!!posts.data && posts.data.pages[0].empty)) {
    return (
      <>{me ?
        value === 0
  ...
  return (
    <div className='bg-p-white'>
      ...
      {posts.isFetchingNextPage ? <LoadingCircle /> : <></>}
    </div>
  )
}
