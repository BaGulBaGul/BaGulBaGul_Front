  ...
  return (
    <div className="flex flex-row w-full px-[16px] py-[10px] gap-[20px]">
      <div className="w-[30px] h-[30px]">
        <AlarmIcn type={props.data.type} checked={props.data.checked} />
      </div>
...
export function AlarmIcn({type, checked}: { type: string, checked: boolean }) {
  switch (type) {
    ...
    case "NEW_RECRUITMENT_LIKE":
      return <AlarmLikeIcn val={checked} />
  }
}

const AlarmLikeIcn = ({val}: { val: boolean }) => {
  return (
    <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="15" cy="15" r="15" fill={val ? '#C1C1C1' : '#4A6AFE'} />
...
const AlarmCmtIcn = ({val}: { val: boolean }) => {
  return (
    <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="15" cy="15" r="15" fill={val ? '#C1C1C1' : '#4A6AFE'} />
...
const AlarmAlertIcn = ({val}: { val: boolean }) => {
  return (
    <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="14.6304" cy="14.9088" r="14.1304" fill={val ? '#C1C1C1' : '#4A6AFE'} />
      <path fill-rule="evenodd" clip-rule="evenodd" d="M13.6488 17.8125L13.6488 7.60719L15.6113 7.60719L15.6113 17.8125L13.6488 17.8125Z" fill="#FCFCFC" />
      <path fill-rule="evenodd" clip-rule="evenodd" d="M13.6488 22.5225L13.6488 20.1674L15.6113 20.1674L15.6113 22.5225L13.6488 22.5225Z" fill="#FCFCFC" />
    </svg>
  )
}