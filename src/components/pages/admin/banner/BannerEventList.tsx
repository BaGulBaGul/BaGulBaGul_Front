...
import { IconRadio } from "@/components/common/styles/Icon";
        ...
        {events.data?.pages.map((event) => (
          event.content.map((item: ListProps, idx: any) => (
            <div key={`event-${idx}`}>
              {idx === 0 ? <></> : <Divider />}
              <BlockWrapper url=""
                blockAction={!!editing && <button className="self-start p-[3px]" onClick={() => updateSelected(item)}><IconRadio checked={selectedId === item.event.eventId} /></button>}
