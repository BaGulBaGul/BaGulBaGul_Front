...
import { CalIcn, ChevronIcn } from "../styles/Icon";
import DatePicker, { registerLocale } from "react-datepicker";
import { ko } from "date-fns/locale/ko";
import { getMonth, getYear } from "date-fns";
import dayjs from "dayjs";
import { HeadSelect, PartiSelect } from "../input/Select";
import { FilterProps } from "..";
import { viewTheme, viewCheckTheme } from "./ViewFilterTheme";
...
interface ViewSelectProps { p: FilterProps; setP: any; open: boolean; setOpen: any; routeToFilter?: any; }
export function ViewSelect(props: ViewSelectProps) {
  ...
  return (
    <ThemeProvider theme={viewTheme}>
      <Dialog open={props.open} onClose={handleClose} scroll="paper">
        <DialogTitle>바글바글 필터</DialogTitle>
        <DialogContent dividers={true}>
          {props.p.proceeding === undefined ? <></> : <FilterCheck title='종료된 행사 제외하기' checked={props.p.proceeding} handleChange={handleProceeding} />}
          {props.p.recruiting === undefined ? <></> : <FilterCheck title='모집 중만 보기' checked={props.p.recruiting} handleChange={handleRecruiting} />}
          ...
          <div className="flex flex-col" id="filter-date">
            <div className="flex flex-row gap-[16px]">
              ...
              <button onClick={handleOpenCal} className={`filter-btn px-[8px] gap-[8px] ${!!startDate ? 'border-primary-blue' : ''}`}>
                <span className={!!startDate ? "text-primary-blue" : ''}><CalIcn val={false} color="currentColor" /></span>
                <span>{!startDate ? <span>날짜를 선택하세요</span>
          ...
          {props.p.headCount === undefined ? <></> : <HeadSelect openHead={openHead} handleOpenHead={handleOpenHead} headCount={props.p.headCount} setHeadCount={setHeadCount} />}
        </DialogContent>
      </Dialog>
    </ThemeProvider >
  )
...
function FilterCheck(props: { title: string; checked: boolean; handleChange: any; }) {
  return (
    <div className="flex flex-row justify-between pt-[10px]">
      <div className="text-14 pb-[2px]">{props.title}</div>
